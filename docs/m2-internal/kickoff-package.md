...
*M2-01 implementation note (2026-09-23, added after the signature; the signed wording is in `3def287`).* The single root `.env.example` above was refined at the M2-01 code review, which owns file locations ("具体实现文件位置在开工代码评审确定", `m2-spec.md` L41). This is an engineering choice, not a founder ruling. The API and the worker both read a variable named `DATABASE_URL`, each with its own login, so each ships its own names-only example, `apps/api/.env.example` and `apps/worker/.env.example`, and loads its own gitignored `apps/api/.env` or `apps/worker/.env` (`tsx --env-file-if-exists`). The root `.env.example` keeps the web, migrate, bootstrap and test names and points to the other two. The root `.gitignore` rule `!.env.example` admits all three, and `apps/api/src/config.test.ts` and `apps/worker/src/config.test.ts` fail if an app's example and its env schema name different variables, or the example carries a value.
