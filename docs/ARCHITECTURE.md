...
updated: 2026-09-26
implementation_status: m1-prototype-accepted; m2-01-internal-loop-built (api, worker, db, internal page; internal acceptance only); m2-02-identity-built (google sign-in, profiles, session liveness, tester allow-list; internal acceptance and the founder's real-login walk); org-membership-and-saved-user-data-unbuilt
...
| 业务服务与数据 | M2-01 建立：[`apps/api`](../apps/api/README.md) Fastify 5.12.5（`/health`、`/internal/campaigns`、`/internal/worker-health`，以只读运行账号 `wringy_api_login` 读 PostgreSQL）；[`apps/worker`](../apps/worker/README.md) pg-boss 12.33.5（进程心跳与队列往返，运行账号 `wringy_worker_login`）；[`packages/db`](../packages/db/README.md)（迁移 0001–0010、迁移账号与运行账号分权、运行账号不能改写 pg-boss 迁移所依据的数据、API 不接触 `pgboss`、`data_origin` 不可改、夹具种子、本地 embedded PostgreSQL 17 与只接受一次性集群的测试 harness）、[`packages/config`](../packages/config/README.md)（zod 环境变量）、[`packages/contracts`](../packages/contracts/README.md)（响应 schema）；单元测试与真实 PostgreSQL 17 集成测试。M2-02 建立身份：`profiles`（键为已验证令牌的 `sub`，列级授权，`status` 只由运营者改）、`sign_in_allowlist`（审计式 CLI 维护、首次登录一次核对）与 `platform.session_is_live` 引导；`apps/api` 以 ES256 JWKS 校验 Supabase 访问令牌（`/identity/sign-in`、`/me`、`/me/session/probe`），每条命令在自己的事务内核实会话存活（数据库或 Auth 服务器两种适配器）。尚无组织、成员、能力授予或保存的业务数据；演示规则仍在 [`apps/web/src/domain`](../apps/web/src/domain/)，演示状态只存浏览器 localStorage | M2-03 起建组织授权与审计，M2-05 起保存业务数据，M3 在服务端事务重验全部资金规则；模块路径按实际实现建立 |
...
现状口径：2026-09-26 本地受版本控制文件（`git ls-files`）、上述 README／manifest／workflow、`main` 上 M2-01 与 M2-02 的代码与测试和 [M2 验收记录](m2-internal/acceptance-record.md)、`main` 分支保护（`gh api repos/BELCORT-SDN-BHD/wringy/branches/main/protection`）、[M1 验收记录](m1-prototype/acceptance-record.md)与创办人验收 [#9](https://github.com/BELCORT-SDN-BHD/wringy/issues/9)，以及 [仓库迁移记录](repository-status.md)。M1 未公开部署；外部部署若无证据即为未验证。`implementation_status` 有四个标记。`m1-prototype-accepted` 只指显式模拟原型。`m2-01-internal-loop-built` 指 M2-01 的 api、worker、db 与内部页已建成，只经内部验收，未部署；它在 PR #82 合并进 `main` 后成立。`m2-02-identity-built` 指 Google 登录、`profiles`、会话存活核实与测试者名单已建成并经创办人真实登录走通（[#21](https://github.com/BELCORT-SDN-BHD/wringy/issues/21)，PR #86／#88），仍只在本机与 CI 运行，未部署。`org-membership-and-saved-user-data-unbuilt` 指组织、成员、能力授予、审计日志与保存的业务数据尚未建立，任务与资金运行也未建立。四者都不抹去已有设计资产与规划基础。
