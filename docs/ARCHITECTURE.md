...
updated: 2026-09-26
implementation_status: m1-prototype-accepted; m2-01-internal-loop-built (api, worker, db, internal page; internal acceptance only); m2-02-identity-built (google sign-in, profiles, session liveness, tester allow-list; internal acceptance and the founder's real-login walk); m2-03-org-membership-built (orgs, memberships, invitations, script-only capability grants, append-only audit log; internal acceptance, simulated identity); saved-business-data-unbuilt
...
| UI 工程 | [`apps/web`](../apps/web/README.md)：Next.js 16.3.5 App Router，两个根布局：`(demo)` 为三端原型（官方 shadcn radix-nova 组件、next-intl 三语、zustand persist 本地演示状态），`(internal)` 为内部版本 `/internal` 页（M2-01，[#16](https://github.com/BELCORT-SDN-BHD/wringy/issues/16)：服务端组件经 Fastify 读取夹具活动与 worker 健康状态，三语、常驻内部版本标识，不挂载演示状态）；Vitest＋Playwright（M1 套件与 M2-AC01 内部套件）；`output: 'standalone'` 供 web 镜像；Vite [设计系统展示](../phase-0/foundation/design-system-v2/app/package.json)另存，不是三端业务应用 | M1 原型已经创办人验收（2026-09-23，[#9](https://github.com/BELCORT-SDN-BHD/wringy/issues/9)，[验收记录](m1-prototype/acceptance-record.md)），全部显式模拟；`/internal` 自 M2-02（[#21](https://github.com/BELCORT-SDN-BHD/wringy/issues/21)）起要求 Google 登录：`proxy.ts` 刷新会话并把访问令牌交给服务端组件，`/auth/*` 路由处理器负责登录、回调与退出（模式守卫、Origin 规则、no-store），三语登录页与结果提示；创办人已用真实 Google 账号走通（2026-09-26）；仍未部署；后续阶段复用页面，改接 Fastify 业务路由 |
| 业务服务与数据 | M2-01 建立：[`apps/api`](../apps/api/README.md) Fastify 5.12.5（`/health`、`/internal/campaigns`、`/internal/worker-health`，以只读运行账号 `wringy_api_login` 读 PostgreSQL）；[`apps/worker`](../apps/worker/README.md) pg-boss 12.33.5（进程心跳与队列往返，运行账号 `wringy_worker_login`）；[`packages/db`](../packages/db/README.md)（迁移 0001–0016、迁移账号与运行账号分权、运行账号不能改写 pg-boss 迁移所依据的数据、API 不接触 `pgboss`、`data_origin` 不可改、夹具种子、本地 embedded PostgreSQL 17 与只接受一次性集群的测试 harness）、[`packages/config`](../packages/config/README.md)（zod 环境变量）、[`packages/contracts`](../packages/contracts/README.md)（请求与响应 schema）；单元测试与真实 PostgreSQL 17 集成测试。M2-02 建立身份：`profiles`（键为已验证令牌的 `sub`，列级授权，`status` 只由运营者改）、`sign_in_allowlist`（审计式 CLI 维护、首次登录一次核对）与 `platform.session_is_live` 引导；`apps/api` 以 ES256 JWKS 校验 Supabase 访问令牌（`/identity/sign-in`、`/me`、`/me/session/probe`），每条命令在自己的事务内核实会话存活（数据库或 Auth 服务器两种适配器）。M2-03 建立组织授权与审计：迁移 0011–0016（`orgs` 记录创建者，`org_members`、`org_invitations`、`admin_scopes`、`platform_grants`、只增的 `audit_log`），运行账号只有所需的列级写权限，不能删除成员、邀请或审计行，也不能写能力授予；`apps/api` 新增 `/me/workspaces`、`/orgs/…` 与 `/invitations/…`：每次请求按路径中的组织从 PostgreSQL 重核成员与角色，每条命令先锁组织行（一个全局锁顺序），邀请为 7 天单次链接（只存令牌的 sha256，按已验证邮箱接受），每次授权拒绝写一条不含密钥的审计行；review、finance、ops_runtime 能力只由审计式 `pnpm db:grant` 以迁移账号写入；`/internal` 增加工作区、组织与邀请页。尚无草稿、投稿等保存的业务数据；演示规则仍在 [`apps/web/src/domain`](../apps/web/src/domain/)，演示状态只存浏览器 localStorage | M2-05 起保存业务数据，M3 在服务端事务重验全部资金规则；模块路径按实际实现建立 |
...
现状口径：2026-09-26 本地受版本控制文件（`git ls-files`）、上述 README／manifest／workflow、`main` 上 M2-01 与 M2-02 的代码与测试、`feat/m2-03` 分支的代码与测试和 [M2 验收记录](m2-internal/acceptance-record.md)、`main` 分支保护（`gh api repos/BELCORT-SDN-BHD/wringy/branches/main/protection`）、[M1 验收记录](m1-prototype/acceptance-record.md)与创办人验收 [#9](https://github.com/BELCORT-SDN-BHD/wringy/issues/9)，以及 [仓库迁移记录](repository-status.md)。M1 未公开部署；外部部署若无证据即为未验证。`implementation_status` 有五个标记。`m1-prototype-accepted` 只指显式模拟原型。`m2-01-internal-loop-built` 指 M2-01 的 api、worker、db 与内部页已建成，只经内部验收，未部署；它在 PR #82 合并进 `main` 后成立。`m2-02-identity-built` 指 Google 登录、`profiles`、会话存活核实与测试者名单已建成并经创办人真实登录走通（[#21](https://github.com/BELCORT-SDN-BHD/wringy/issues/21)，PR #86／#88），仍只在本机与 CI 运行，未部署。`m2-03-org-membership-built` 指组织、成员、邀请、脚本式能力授予与只增审计日志已建成（[#26](https://github.com/BELCORT-SDN-BHD/wringy/issues/26)），只经内部验收：数据库与 API 行在真实 PostgreSQL 17 上运行，身份为模拟，真实 Google 走查待创办人决定，未部署；它在 `feat/m2-03` 合并进 `main` 后成立。`saved-business-data-unbuilt` 指草稿、投稿等保存的业务数据尚未建立，任务与资金运行也未建立。五者都不抹去已有设计资产与规划基础。
