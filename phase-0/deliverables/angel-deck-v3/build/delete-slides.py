...
keep=list(range(1,10))+[12];removed=[10,11,13,14,15,16,17];remove_parts=set();rows=[]
with ZipFile(original) as z:
 ...
 if 'docProps/app.xml' in z.namelist():changes['docProps/app.xml']=re.sub(rb'(<Slides>)\d+(</Slides>)',rb'\g<1>10\2',z.read('docProps/app.xml'))
 with ZipFile(build/'candidate-v3.pptx','w') as dst:
...
writer.add_metadata({'/Title':'Wringy 天使轮讨论稿 v3','/Author':'Wringy'})
with (out/'Wringy-Angel-Deck-v3.pdf').open('wb') as f:writer.write(f)
...
print('Kept original pages1–9 and12; removed10,11 and allappendices; 10pages.')