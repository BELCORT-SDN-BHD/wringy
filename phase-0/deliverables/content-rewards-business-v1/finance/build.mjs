...
const dir=new URL('.',import.meta.url).pathname,out=dir+'outputs/initial-funding/';
...
const ds=[
 ...
 ['创办人津贴／月：基础',i.founder_allowance_base_monthly,i.founder_living_support_status],
 ['创办人津贴／月：敏感性',i.founder_allowance_sensitivity_monthly,'仅敏感性，不是已批准薪资'],
...
title(p,'Content Rewards 输入','蓝字黄底可改。RM；全部非上限项目为内部预算或演示假设。');hdr(p,'C5:D5',['输入','数值']);v(p,'F5','依据与边界');
...
const cl={6:'月序号',7:'研发总额（含QA及内部预备金）',8:'地推与交通',9:'付费运营支持池',10:'云与工具',11:'持续维护（排除缺陷保修）',12:'法务与会计',13:'试点数据与付款费池',14:'创办人津贴',15:'非研发支出',16:'支出合计',18:'收入抵扣',19:'期初公司现金',20:'拟议融资到账',21:'已承诺资本到账',22:'期末公司现金',25:'研发总上限',26:'非研发支出合计',27:'全部支出',28:'非研发现金缓冲',29:'减：已承诺资本',30:'减：已核实现金',31:'基础最低资金需求',32:'基础取整募资建议',33:'取整增加储备',34:'敏感性：六个月创办人津贴',35:'敏感性：非研发支出',36:'敏感性：非研发缓冲',37:'敏感性：资金需求',38:'敏感性：取整建议',39:'基础期末现金',41:'研发实施额度（内部预备金前）'};
...
const cf={25:'=D7',26:'=D15',27:'=D16',28:"=D26*'Inputs'!D8",29:"='Inputs'!D22",30:"='Inputs'!D23",31:'=MAX(0,D27+D28-D29-D30)',32:"=ROUNDUP(D31/'Inputs'!D21,0)*'Inputs'!D21",33:'=D32-D31',34:"='Inputs'!D10*6",35:'=D26-D14+D34',36:"=D35*'Inputs'!D8",37:'=MAX(0,D25+D35+D36-D29-D30)',38:"=ROUNDUP(D37/'Inputs'!D21,0)*'Inputs'!D21",39:'=J22',41:"=D25-'Inputs'!D7"};for(const[k,x]of Object.entries(cf))f(c,'D'+k,x);
...
for(const rr of [15,16,22,31,32,37,38])c.getRange(`C${rr}:J${rr}`).format.fill='#EEF1E7';
...
const su={6:['研发总上限（内含预备金）',25],7:['地推、运营及其他非研发',26],8:['非研发现金缓冲',28],9:['基础资金需求',31],10:['基础取整募资建议',32],11:['基础期末公司现金',39],14:['敏感性：创办人津贴六个月',34],15:['敏感性：资金需求',37],16:['敏感性：取整募资建议',38]};for(const[k,[l,t]]of Object.entries(su)){v(s,'C'+k,l);f(s,'D'+k,`='Cash'!D${t}`);}
...
v(s,'C19','零津贴假设个人生活由创办人自筹，尚未确认；上方同时显示每月RM6,000的影响。');v(s,'C21','研发RM50,000已包含测试、交接及内部预备金，不另加15%研发缓冲。');v(s,'C23','若报价超过上限，减／延后非必要范围；不得降低安全与可用beta的验收要求。');v(s,'C25','不以假定收入抵扣募资；品牌奖励池不计公司收入或可用现金。');v(s,'C27','试点单场现金成本消耗已列预算池，不额外扣一次贡献成本。');v(s,'C29','单场15%费率仅演示，非已批准定价。没有承诺未来轮金额或回本日期。');
s.getRange('F1:F30').format.columnWidth=40;s.getRange('G1:G30').format.columnWidth=18;s.getRange('D6:D16').setNumberFormat(nf);s.getRange('G6:G11').setNumberFormat(nf);for(const rr of [9,10,15,16]){s.getRange(`C${rr}:D${rr}`).format.fill=rr===10?'#DEE8B5':'#EEF1E7';s.getRange(`C${rr}:D${rr}`).format.font.bold=true;}
v(s,'C10','零津贴：暂定取整情景');v(s,'C16','含津贴：暂定取整情景');v(s,'C18','两种资金情景均为暂定，尚未选择募资额。');v(c,'C32','零津贴：暂定取整情景');v(c,'C38','含津贴：暂定取整情景');
...
v(p,'D9',6000);w.recalculate();eq('actual allowance edit feeds cash',get(c,'D31'),107063);v(p,'D9',0);
