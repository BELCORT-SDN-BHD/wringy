...
// 9 Modeled reward spend, captured share, annual service revenue
s=page({sub:false});text(s,'电商商家子市场估算',58,151,540,48,30,true,C.primary);text(s,'约 RM150m / 年',58,225,576,89,53,true,C.primary);text(s,'奖励支出潜力',58,326,535,49,29);text(s,'约6,259个假设采购方\n× 年奖励RM24,000',58,415,516,116,30,false,muted);
text(s,'Wringy年服务收入',642,159,571,49,32,true,C.primary);text(s,'取得奖励支出份额 × 15%服务费',642,220,572,48,25,false,muted);
chart(s,'bar',{position:box(642,304,571,270),categories:['1%','3%','5%'],series:[{name:'年服务收入（千RM）',values:[225.31968,675.95904,1126.5984],fill:C.primary}],barOptions:{direction:'bar',grouping:'clustered'},hasLegend:false,xAxis:axis,yAxis:{...hidden,min:0,max:1350},dataLabels:{showValue:false}});
text(s,'1% ≈ RM225k   3% ≈ RM676k   5% ≈ RM1.13m',618,590,602,40,22,true,C.primary);link(s,'dosm','来源：DOSM电商经营单位',9,58,640,480);
...
await fs.writeFile(path.join(BUILD,'source-links.json'),JSON.stringify(links,null,2));await (await PresentationFile.exportPptx(p)).save(path.join(BUILD,'draft.pptx'));console.log('v11 draft:14 slides,4 native charts,4 concept diagrams');
