...
with zipfile.ZipFile(deck) as z:
 ...
 cv=[[float(t.text) for t in x.findall('.//'+C+'numCache/'+C+'pt/'+C+'v')] for x in charts];assert cv==[[2000,300],[0,8,18,30,40],[225.31968,675.95904,1126.5984],[50000,13620,6380]],cv
...
with zipfile.ZipFile(r.parent/'content-rewards-business-v10/output/Wringy-Content-Rewards-Business-Plan.pptx') as z:
 old_texts=['\n'.join(t.text or '' for t in E.fromstring(z.read(f'ppt/slides/slide{i}.xml')).iter(A+'t')) for i in range(1,15)]
 assert all(texts[i]==old_texts[i] for i in range(14) if i!=8),'Unintended visible copy change'
 assert all((r/'build/renders'/f'slide-{i:02}.png').read_bytes()==(r.parent/'content-rewards-business-v10/build/renders'/f'slide-{i:02}.png').read_bytes() for i in range(1,15) if i!=9)
...
old=json.loads((r.parent/'content-rewards-business-v10/delivery-check.json').read_text());assert sha(f)==old['finance_sha256']
