...
assert len(imgs)==10
pdf=root/'output/Wringy-Investor-v4-Preview.pdf'
c=canvas.Canvas(str(pdf),pagesize=(960,540))
c.setTitle('Wringy 投资讨论稿 v4');c.setAuthor('Wringy')
for fn in imgs:
 c.drawImage(ImageReader(str(fn)),0,0,width=960,height=540);c.showPage()
c.save()
w,h,gap,cols=480,270,16,2
sheet=Image.new('RGB',(cols*w+(cols+1)*gap,5*h+6*gap),'white')
...
ppt=root/'output/Wringy-Investor-v4.pptx'
with zipfile.ZipFile(ppt) as z:
 slides=[n for n in z.namelist() if n.startswith('ppt/slides/slide') and n.endswith('.xml')]
 charts=[n for n in z.namelist() if '/charts/chart' in n and n.endswith('.xml')]
 assert len(slides)==10
 assert len(charts)>=2
report={'slides':10,'pdf_pages':10,'pdf_type':'raster preview from final PPTX slide renders','native_charts':len(charts),'limitations':['No PowerPoint desktop edit-save-reopen test'],'files':{p.name:{'bytes':p.stat().st_size,'sha256':hashlib.sha256(p.read_bytes()).hexdigest()} for p in [ppt,pdf,root/'output/contact-sheet.png']}}
