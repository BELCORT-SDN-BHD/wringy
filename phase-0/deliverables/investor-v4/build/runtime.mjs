...
export async function finalize(candidatePath,finalPath,revision='final'){
 return finalizePresentation({workspaceDir:ROOT,candidatePath,finalPath,pythonExecutable:PY,
  ...
  explicitTotalSlideCount:10,requiredNativeTableOwnerSlides:[],requiredNativeChartOwnerSlides:[7,10],
  materializeLiteralChartWorkbooks:true,fontPolicy:{basis:'user_request',families:[FONT]},verifyArtifactToolImport:true,
...
export async function renderFinal(finalPath){
 ...
 if(deck.slides.items.length!==10)throw Error('Expected ten slides');
