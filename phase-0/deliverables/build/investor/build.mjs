...
await fs.writeFile(path.join(here,'.private/input-snapshot.json'),raw);
...
function table(sl,s){
 const headers=s.table.headers.map(v=>s.id==='APP05'?String(v).replace('经营利润（MYR）','经营利润\n（MYR）').replace('净烧钱（MYR）','净烧钱\n（MYR）'):v),rows=s.table.rows.map(r=>r.map(v=>s.id==='INV12'?String(v).replace('RM1.8m','RM180万').replace('RM18m','RM1800万').replace('RM126m','RM1.26亿'):v)),n=headers.length;if(rows.length>8)throw Error(`${s.id}: pagination required`);
 const widths=({INV08:[200,180,130,642],INV16:[350,230,572],INV18:[190,370,592],INV21:[240,120,400,392],INV23:[340,330,482],INV26:[380,350,422],APP01:[250,490,412],APP06:[440,244,234,234],APP08:[260,540,352]})[s.id]??({3:[340,430,382],4:[252,300,300,300],5:[210,210,252,240,240]})[n];
...
for(const [idx,s]of source.slides.entries()){
 ...
 const notes=[s.takeaway,s.speaker_notes_zh,...s.body,s.example,...(s.table?[s.table.headers.join("；"),...s.table.rows.map(r=>r.join("；"))]:[]),...(s.sources??[]).map(x=>`${x.title}\n${x.url}`)];
 ...
 if(s.layout==='cover'){
  ...
  if(s.table)table(sl,s);
   ...
   const a=assets[s.id];
   box(sl,'产品概念·示例数据',64,202,1152,42,22,theme.purple,true);
   await picture(sl,a[0],64,248,600,417);await picture(sl,a[1],690,234,200,432);
   ['品牌端：写清要求，\n保留审核依据。','创作者端：确认报酬，\n查看下一步。','品牌直接付款；\n审核批准不等于到账。'].forEach((v,j)=>box(sl,v,928,266+j*126,288,110,24));
   for(const im of a)notes.push(im.source?`图片来源：${im.source}`:'');
   notes.push('工作台截图为产品概念，所示品牌、活动、金额及状态均为示例数据；不代表已上线产品或真实交易。');
  ...
  if(s.example&&s.id!=='INV05')box(sl,s.example.replace(/\s+/g,' '),64,607,1152,70,23,dark?theme.lime:theme.muted);
 }
...
let version=1,output=path.join(root,'output/02-Wringy东南亚商业计划.pptx');while(true){try{await fs.access(output);output=path.join(root,`output/02-Wringy东南亚商业计划-v${++version}.pptx`);}catch{break;}}
...
await fs.writeFile(path.join(here,'coverage.json'),JSON.stringify(coverage,null,2));
