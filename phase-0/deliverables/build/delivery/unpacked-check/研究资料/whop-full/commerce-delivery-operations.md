...
此前六组流程的记录见先前证据（工作区参考，未随包附带）：开户、定价、搜索、退款申请、affiliate、购买。本轮只把它们作为历史指针；没有把旧文中的观察冒称本轮重新查看，新的具体功能以正文和上述三组图补强。发现/评价/affiliate 不归本文拥有。
...
- 第一批官方正文（工作区参考，未随包附带）：业务家族、账号/团队、目录、课程、顾客运营、第三方集成等。
- 补充接口/费用正文（工作区参考，未随包附带）：Current 删除/运单/文件、社群管理、课程删除、客服、稳定性、收费。
- 移动访问正文（工作区参考，未随包附带）：iOS、Android、移动浏览器与外部服务步骤。
- 全目录指针（工作区参考，未随包附带）：只作发现索引，不作 993 个功能的证据。
