...
import {loadFinance,financeMetadata,cashMetadata,fillUnitEconomics,fillMonthlyProfit,fillOperatingBreakEven,fillCashPayback,fillCashAssumptions} from './finance-slides.mjs';
import sharp from 'sharp';
import {fs,path,Presentation,PresentationFile,ROOT,BUILD,sha,applyPresentationChartFont} from './runtime.mjs';
...
const C={background:c.canvas,surface:c.surface,ink:c.ink,accent:c.citron,forest:c.forest,muted:c.secondary,line:c.line,focus:c.focus};
...
const model=await loadFinance(fs,path,ROOT),d=model.d,financial=financeMetadata(d),cash=cashMetadata(model);
for(const m of financial)m.sources=model.sources;
const scale=model.results.scenarios.find(a=>a.name==='扩张').months.find(m=>m.campaigns===200);
const fm=n=>new Intl.NumberFormat('en-US',{maximumFractionDigits:0}).format(n);
{
...
const rows=[['商家提供授权素材','自有直播／产品视频，说明店铺与推广目标'],['创作者剪辑并发布','多人制作不同片段，面向约定受众传播'],['核验合格播放','按期间、受众与内容规则，排除重复和作弊'],['按成果计算奖励',`每 1,000 次合格播放 RM${model.v.cpm}（示例费率）`]];rows.forEach(([a,b],i)=>{tx(s,String(i+1).padStart(2,'0'),64,199+i*96,70,48,31,true);tx(s,a,151,197+i*96,1060,49,31,true);tx(s,b,151,246+i*96,1050,42,24,false,C.muted);});
tx(s,`个人示例：50,000 次合格播放 = RM${fm(50000/1000*model.v.cpm)} 奖励（须在限额内）`,64,603,1152,43,26,true);
}
{
...
{
 const m=reviewPages[0],s=page(6,m.title,m.foot,m.note,m.sources);
 await fillReviewRoles({s,tx,line,ico,C,d,model,applyPresentationChartFont});
}
{
 const m=reviewPages[1],s=page(7,m.title,m.foot,m.note,m.sources);
 await fillReviewTiming({s,tx,line,ico,C,d,model,applyPresentationChartFont});
}
{
 const m=financial[0],s=page(8,m.title,m.foot,m.note,m.sources);
 await fillUnitEconomics({s,tx,line,ico,C,d,model,applyPresentationChartFont});
}
{
 const m=financial[1],s=page(9,m.title,m.foot,m.note,m.sources);
 await fillMonthlyProfit({s,tx,line,ico,C,d,model,applyPresentationChartFont});
}
{
 const m=financial[2],s=page(10,m.title,m.foot,m.note,m.sources);
 await fillOperatingBreakEven({s,tx,line,ico,C,d,model,applyPresentationChartFont});
}
{
 const m=cash[0],s=page(11,m.title,m.foot,m.note,m.sources);
 await fillCashPayback({s,tx,line,ico,C,d,model,applyPresentationChartFont});
}
{
 const m=cash[1],s=page(12,m.title,m.foot,m.note,m.sources);
 await fillCashAssumptions({s,tx,line,ico,C,d,model,applyPresentationChartFont});
}
{
const s=page(13,'复购品牌与持续参与的创作者','增长假设 · 尚无经核验复购、留存或规模化数据','假设品牌获得可核验的合格分发后重复采购，创作者在收益合理时持续参与。试点测量品牌复购率、每月活动数、活跃创作者与次月参与率、核准奖励、每千次合格播放总成本、审核成本和作弊／争议损失。不声称网络效应已经成立。',[framing]);
...
{
const s=page(14,'视频电商的市场背景','来源：Google／Temasek／Bain，2025年估算 · 两项地域口径不同，不相乘，非Wringy收入空间','2025年估算东南亚视频电商占电商交易额25%，75%为算术余项。马来西亚全部电商GMV估算US$20 billion=US$200亿。两项不可相乘，也不是创作者奖励市场或Wringy收入空间。',[MARKET.regionalSource,MARKET.malaysiaSource]);
...
{
const s=page(15,'收入规模的算式','数学情景，非预测 · 需求与4亿次播放供给未验证 · 成本随规模另测','月平台收入=活跃付费品牌数×每品牌每月活动数×每场核准奖励×费率。示例100×2×10000×15%=300000MYR。对应200场/月、2000000MYR核准奖励，按RM5/1000需400000000合格播放/月。这些规模和数据可得性完全未验证。仅计算Rewards平台服务收入，不是利润，未加入未来商业工具费用。数据读取现金情景扩张路径200场月份以保持口径。规模需求仍未验证，成本与供给须实测。',[framing]);
const f=[['活跃品牌',fm(scale.activeBrands)],['月活动／品牌',`× ${model.v.frequency}`],['核准奖励／场',`× RM${fm(model.v.reward)}`],['平台费率',`× ${model.v.fee*100}%`]];f.forEach(([a,b],i)=>{const x=64+i*291;tx(s,a,x,225,275,45,24);tx(s,b,x,299,285,72,i===2?36:52,true);});
line(s,64,407,1152);tx(s,`RM${fm(scale.revenue)}／月`,64,451,865,103,69,true);tx(s,'示例平台服务收入',67,573,650,43,28);tx(s,`每月${fm(scale.campaigns)}场活动\nRM${fm(scale.campaigns*model.v.reward/10000)}万核准奖励\n${scale.eligibleViews/100000000}亿次合格播放`,891,469,325,123,24);
}
{
const s=page(16,'第一阶段 Rewards，随后扩展商业工具','后续均为收入假设 · 上线范围、收费及支付能力未定 · 不把交易额计作平台收入','第一阶段拟以按合格成果奖励的Creator Rewards验证品牌付费与创作者供给。未来参考Whop式完整商业平台：支付及交易服务、店铺和数字产品、课程与会员。候选收入杠杆为交易服务费、订阅、工具服务费。均非已上线能力、承诺路线图或已验证收入。对同一交易仅按最终约定费用确认收入，不叠加重复计算。',['https://whop.com/',framing]);
...
{
const s=page(17,'资金规划参考：RM576,150','原按帖方案预算锚点；按播放计酬需重新估价 · 非正式融资额或报价，税费与交易成本待补','保留历史固定获批帖MVP的18个月零收入基准：189750开发含预备金+216000商业运营18个月+70000上线后技术14个月+50000获客启动+50400非开发15%缓冲=576150。前4个月建设、后14个月运营。此预算不是按播放/KPI计酬方案的有效报价。新增数据接入、合格判断、反作弊、奖励重算等须重新估价。完整Whop扩展未包含。融资金额与结构另定。',['phase-0/foundation/belcort-handoff-v1.md',framing]);
...
{
 const m=reviewPages[2],s=page(18,m.title,m.foot,m.note,m.sources);
 await fillReviewAppendix({s,tx,line,ico,C,d,model,applyPresentationChartFont});
}

notes[3].sources.push(...model.sources);notes[14].sources.push(...model.sources);
...
await fs.writeFile(path.join(BUILD,'records.json'),JSON.stringify({tokenSource:canonical,tokenHash:sha(raw),slides:records,budgetTotal:BUDGET_TOTAL,financeSources:model.sources,financeStatus:'frozen',financeManifest:model.manifest},null,2));
await fs.writeFile(path.join(ROOT,'story.md'),'# Wringy 投资讨论稿 v6\n\n财务直接读取model-v1结果。研究事实与Wringy提案分开。财务与冻结model-v1一致。\n\n'+notes.map((n,i)=>`## ${i+1}. ${records[i].title||'Wringy'}\n\n${n.text}\n\n来源：${n.sources.join('\n')}\n`).join('\n'));
if(p.slides.items.length!==18)throw Error('Expected18');
await(await PresentationFile.exportPptx(p)).save(path.join(BUILD,'candidate.pptx'));
for(let i=0;i<18;i++){const b=await p.export({slide:p.slides.items[i],format:'png',scale:1});await fs.writeFile(path.join(BUILD,'draft-renders',`slide-${String(i+1).padStart(2,'0')}.png`),new Uint8Array(await b.arrayBuffer()));console.log('Rendered draft',i+1);}
