...
const allocation=JSON.parse(await fs.readFile(path.join(ROOT,'funding-allocation.json')));
...
// 9 Actual statistical base and explicit assumed filters
s=page({sub:false});text(s,'电商商家子市场估算',58,145,1157,45,29,true,C.primary);
text(s,'DOSM 2022统计',58,223,370,37,23,false,muted);text(s,'78,236',58,274,348,72,49,true,C.primary);text(s,'电商经营单位',58,356,352,43,25);
text(s,'×',410,280,70,60,40,false,muted);text(s,'活跃调整 · 假设',516,223,303,37,23,false,muted);text(s,'80%',516,274,290,72,49,true,C.primary);
text(s,'×',815,280,70,60,40,false,muted);text(s,'适配比例 · 假设',920,223,303,37,23,false,muted);text(s,'10%',920,274,290,72,49,true,C.primary);
rule(s,58,423,1160);text(s,'≈ 6,259',58,455,372,76,49,true,C.primary);text(s,'潜在采购方情景',58,544,359,46,26);
text(s,'× 月奖励RM2,000\n× 12个月',471,464,342,104,29,false,muted);text(s,'≈ RM150m / 年',859,464,370,65,37,true,C.primary);text(s,'奖励支出潜力',859,543,363,46,26);link(s,'dosm','来源：DOSM电商经营单位',9,58,640,480);
// 10 Merchant counts and annual service income, continuous spending scenario
s=page({sub:false});text(s,'每商家平均每月',58,166,535,49,31,true,C.primary);text(s,'已确认奖励 RM2,000',58,235,565,55,32);text(s,'× 15%服务费',58,305,543,54,30,false,muted);text(s,'= RM300 / 月',58,384,557,70,44,true,C.primary);text(s,'RM3,600 / 年',58,478,557,67,41,true,C.primary);
text(s,'Wringy年服务收入（千RM）',649,167,564,49,29,true,C.primary);
chart(s,'bar',{position:box(646,268,572,303),categories:['50家','100家','300家'],series:[{name:'年服务收入（千RM）',values:[180,360,1080],fill:C.primary}],barOptions:{direction:'column',grouping:'clustered'},hasLegend:false,xAxis:{...hidden},yAxis:{...axis,min:0,max:1250},dataLabels:{showValue:true,position:'outEnd',textStyle:{fontSize:23,fill:C.primary}}});
text(s,'50家',695,579,136,40,24,true,C.primary);text(s,'100家',889,579,136,40,24,true,C.primary);text(s,'300家',1083,579,136,40,24,true,C.primary);
...
s=page({sub:false});text(s,'拟议',58,157,386,51,29,false,muted);text(s,'RM70,000',58,225,587,105,68,true,C.primary);text(s,'本阶段目标 2–4个月',58,375,499,62,33);text(s,'研发与付费试点验证',58,449,499,62,30,false,muted);
chart(s,'doughnut',{position:box(607,161,365,365),categories:['研发','储备资金'],series:[{name:'资金用途RM',values:[allocation.research_and_development,allocation.reserve_fund],points:[{idx:0,fill:C.primary},{idx:1,fill:C.brand}]}],hasLegend:false,doughnutOptions:{holeSize:70,firstSliceAngle:270}});
text(s,'资金用途',690,320,200,50,27,true,C.primary);rect(s,950,204,16,16,C.primary);rect(s,950,385,16,16,C.brand);text(s,'50,000  研发',976,191,253,47,28,true,C.primary);text(s,'原型与可用Beta',976,245,253,88,23,false,muted);text(s,'20,000',976,372,253,47,28,true,C.primary);text(s,'储备资金',976,426,253,47,28,true,C.primary);
...
await fs.writeFile(path.join(BUILD,'source-links.json'),JSON.stringify(links,null,2));await (await PresentationFile.exportPptx(p)).save(path.join(BUILD,'draft.pptx'));console.log('Rebuilt source for funding slide: 50000 + 20000');
