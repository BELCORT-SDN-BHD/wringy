...
assert len(imgs)==12
pdf=root/'output/Wringy-Investor-v5-Preview.pdf'
c=canvas.Canvas(str(pdf),pagesize=(960,540))
c.setTitle('Wringy 投资讨论稿 v5');c.setAuthor('Wringy')
for fn in imgs:
 c.drawImage(ImageReader(str(fn)),0,0,width=960,height=540);c.showPage()
c.save()
w,h,gap,cols=480,270,16,2
sheet=Image.new('RGB',(cols*w+(cols+1)*gap,6*h+7*gap),'white')
...
assert len(PdfReader(str(pdf)).pages)==12
ppt=root/'output/Wringy-Investor-v5.pptx'
with zipfile.ZipFile(ppt) as z:
 ...
 assert len(charts)>=2
report={'slides':12,'pdf_pages':12,'pdf_type':'raster preview from final PPTX slide renders','native_charts':len(charts),'limitations':['No PowerPoint desktop edit-save-reopen test'],'files':{p.name:{'bytes':p.stat().st_size,'sha256':hashlib.sha256(p.read_bytes()).hexdigest()} for p in [ppt,pdf,root/'output/contact-sheet.png']}}
